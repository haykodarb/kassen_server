  ...
  try {
    ...
    if (!req.body || typeof req.body.temperature !== "number") {
      return res.status(400).json({
        error:
          "Invalid request. Please provide a numeric temperature value in JSON format.",
    ...
    let stmt = db.prepare(
      `INSERT INTO readings (timestamp, sensor_id, temperature) VALUES (?, ?, ?)`,
    );

    stmt.run(timestamp, "SENSOR", temperature);
...
app.get("/images", (req, res) => {
  try {
    const stmt = db.prepare(
      "SELECT id, timestamp, sensor_id, image_path FROM images ORDER BY timestamp DESC",
    );
    const rows = stmt.all();
    res.json(rows);
  } catch (err) {
...
app.get("/readings", (req, res) => {
  try {
    const stmt = db.prepare(
      "SELECT id, timestamp, sensor_id, temperature FROM readings ORDER BY timestamp DESC",
    );
    const rows = stmt.all();
    res.json(rows);
  } catch (err) {
